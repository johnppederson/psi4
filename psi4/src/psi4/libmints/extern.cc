...
std::pair<SharedMatrix, SharedMatrix> ExternalPotential::computePotentialGradients(std::shared_ptr<BasisSet> basis, std::shared_ptr<Matrix> Dt) {
    ...
    auto extcgrad = std::make_shared<Matrix>("Gradient on External Charges", nextc, 3);
    double **Gp = grad->pointer();
    double **EGp = extcgrad->pointer();
    ...
    for (int t = 0; t < threads; t++) {
        ...
        Vtemps[t]->zero();
        EVtemps.push_back(SharedMatrix(extcgrad->clone()));
        EVtemps[t]->zero();
    }
...
#pragma omp parallel for schedule(dynamic) num_threads(threads)
    ...
    for (int t = 0; t < threads; t++) {
        grad->add(Vtemps[t]);
        extcgrad->add(EVtemps[t]);
    }
    std::pair<SharedMatrix, SharedMatrix> grads;
    grads.first = grad;
    grads.second = extcgrad;
    return grads;
}
